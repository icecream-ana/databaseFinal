...
本系统为一个智慧物流车队与配送管理系统，采用前后端一体化 Web 架构，后端基于 Flask 框架，前端页面由 Jinja2 模板 + Bootstrap 组成，数据库使用 Microsoft SQL Server，通过表、触发器，存储过程和视图实现核心业务规则与统计分析，并建立索引进行优化。
...
| 项目     | 版本                            |
| ------ | ----------------------------- |
| Python | 3.9 / 3.10（满足 Flask 3.x 运行要求） |
| 虚拟环境管理 | Conda / venv                  |
| 开发工具   | VS Code                       |
| 浏览器    | Chrome（用于页面验证于调试）             |
...
本次系统使用了电脑本地的数据库，通过配置sa账号进行演示，同时通过在`db.py`中硬编码进行连接：
...
同时需要配置SQL Server网络访问：在SQL Server配置中启用TCP/IP，并确认防火墙放行本地连接
